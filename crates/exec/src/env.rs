...
///
/// `GIT_EDITOR` and `GIT_SEQUENCE_EDITOR` are `true`, not `false`: `true` exits
/// 0, so git treats the message as accepted. `false` would fail every merge
/// commit.
...
mod tests {
    ...
    /// — the command does not clear the environment, so `PATH` and credential
    /// helpers stay visible to the child.
