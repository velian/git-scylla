...
#[cfg_attr(feature = "ts", derive(ts_rs::TS), ts(export))]
pub enum ErrorKind {
    /// The engine stopped. The window is dead.
    EngineStopped,
    /// A selection expression the user typed did not parse.
    BadSelection,
    /// The user cancelled a native dialog.
    Cancelled,
    /// Something on the filesystem or in the OS refused.
    Io,
    /// The action needs something the user has not set — an editor, so far.
    NotConfigured,
}
